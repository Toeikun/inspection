...
@section('content')
	<div class="container">
        <form action="{{ url('student/word') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
                        ...
                        <div class="col-lg-6 mb-2">
                            ...
                            <select name="status" class="form-control" required>
                                ...
                                @foreach ($liststatus as $status)
                                    <option value="{{$status->status_id}}" {{ old('status_id') == $status->status_id ? 'selected' : '' }}>{{$status->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
