...
@section('content')
  <div class="container">
    <form class="form-horizontal" action="{{ url('staff/permission'. '/' . $permission->permission_id) }}" method="POST">
      @csrf
      <div class="row">
      ...
      <div class="row mt-2">
        <div class="col-lg-6">
          <div class="card card-block border-0 p-3">
            <input type="text" class="form-control" name="status_id" value="{{ $permission->permission_id }}" readonly hidden/>
            ...
            <div class="form-group mb-3">
              <label>Note</label>
              <textarea class="form-control" name="note" rows="5" required placeholder="Note">{{ $permission->note }}</textarea>
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="card card-block border-0 p-3">
            <div class="form-group mb-3">
              ...
              <input type="text" class="form-control" value="{{ $permission->created_by }}" name="created_by" readonly/>
            </div>
          </div>
        </div>
      </div>
