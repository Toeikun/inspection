...
@section('content')
  <div class="container">
    <form class="form-horizontal" action="{{ route('staff.staff.store') }}" method="POST">
      @csrf
      <div class="row">
            ...
            <div class="form-group mb-3 {{ $errors->has('permission') ? ' has-error' : '' }}">
                ...
                <select name="permission" class="form-control" required>
                    ...
                    @foreach ($permissions as $permission)
                        <option value="{{$permission->permission_id}}" {{ old('permission') == $permission->permission_id ? 'selected' : '' }}>{{$permission->name}}</option>
                    @endforeach
                </select>
